...
- `runtime/Cargo.toml` only when a test-only dependency is required

Do not edit `collector.rs`, `storage.rs`, migrations, `engine`, `contracts`, `app`, `config`,
`render.yaml`, existing workflows, or unrelated documentation.

## Required behavior

- Deserialize order-book, ticker, trade, market-stats, height, subscription acknowledgement, and
  error frames through typed versioned structures.
...
- Invalidate the local book and return a typed reconnect-required error on a continuity gap.
...
- No secret, wallet path, signer dependency, authenticated endpoint, or write API appears in the
  diff.
