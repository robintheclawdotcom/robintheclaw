...
- Slither 0.11.5 covered 51 contracts with 101 detectors. Before three reviewed `reentrancy-balance` suppressions, the first-party scan returned 72 results: 10 high, 17 medium, 35 low, and 10 informational. The ten high reports trace balance baselines across the two guarded vault calls and the adapter's Permit2/router calls. Both vault entrypoints and all balance-moving sibling functions are `nonReentrant`; the adapter accepts only its immutable bound vault, so token, Permit2, and router callbacks cannot satisfy its caller check. Exact function-level suppressions record that review while leaving the detector active everywhere else. The release scan returned 62 first-party results: 0 high, 17 medium, 35 low, and 10 informational.
