...
export default function SettingsPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Settings" title="Account settings" description="Recovery, display, notification, and session controls." />
      <div className="settings-grid">
        <section className="panel settings-card"><div><span className="eyebrow">Recovery</span><h2>{query.data.user.hasRecovery || auth.hasRecovery ? "Recovery connected" : "Add account recovery"}</h2><p>Email or passkey restores this account and its smart-account address.</p></div><div className="button-row"><button className="button button-secondary" onClick={auth.linkEmail}>Add email</button><button className="button button-secondary" onClick={auth.linkPasskey}>Add passkey</button></div></section>
        ...
        <section className="panel settings-card session-card"><div><span className="eyebrow">Session</span><h2>End session</h2><p>Signing out does not change vault ownership or linked-wallet records.</p></div><button className="button button-secondary" onClick={() => void auth.logout()}>Sign out</button></section>
      </div>
