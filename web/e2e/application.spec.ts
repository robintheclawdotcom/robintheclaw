...
test("email or passkey user opens a real empty-position dashboard", async ({ page }) => {
  ...
  await page.goto("/app");
  await expect(page.getByRole("heading", { name: "One account for capital, strategy, and performance." })).toBeVisible();
...
test("one-click onboarding confirms the sponsored batch", async ({ page }) => {
  await mockApplication(page, { withVault: false });
  await page.goto("/app/onboarding");
  await expect(page.getByRole("heading", { name: "Activate Robin in one onchain operation" })).toBeVisible();
...
test("interrupted onboarding resumes from the saved operation", async ({ page }) => {
  ...
  await page.goto("/app/onboarding");
  await expect(page.getByRole("heading", { name: "Your Robin strategy account is ready." })).toBeVisible();
  expect(await page.evaluate(() => localStorage.getItem("robin:onboarding-call-id"))).toBeNull();
});
...
test("wallet conflict opens the account recovery path", async ({ page }) => {
  ...
  await expect(page.getByText("This wallet belongs to another Robin account.")).toBeVisible();
  await expect(page.getByRole("button", { name: "Recover other account" })).toBeVisible();
});
